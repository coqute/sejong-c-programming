...
void buildQ(Vertex* Q, Vertex* start, int s, int n);
// 업힙
...
void DijkstraShortestPaths(Vertex* vertexL, int s, int n);
...
int main() {
	...
	DijkstraShortestPaths(vertexL, s, n);
...
// 출발정점 s에서 다른 모든 정점으로의 최단거리르 정점 번호의 오름차순으로 출력
void DijkstraShortestPaths(Vertex* vertexL, int s, int n) {
	...
	while (size != 0) {
		...
		for (Incidence* iPtr = u.incidence->next; iPtr != NULL; iPtr = iPtr->next) {
			...
			for (int i = 1; i <= size; i++) {
				if (Q[i].num == z->num && (u.distance + iPtr->line->weight) < z->distance) {
					z->distance = u.distance + iPtr->line->weight;
	...
	ptr = vertexL;
	while (ptr != NULL) {
		if (ptr->num != s && ptr->distance != 30000)
